...
class RegisterRequest(BaseModel):
    org_name: str; org_industry: Optional[str] = None
...
class UserResponse(BaseModel):
    id: str; email: str; full_name: Optional[str]; role: str
    org_id: str; org_name: str; org_plan: str; is_verified: bool
...
async def _unique_slug(db, base):
    ...
    while True:
        ...
        if not r.scalar_one_or_none(): return candidate
        suffix += 1

@router.post("/register", response_model=TokenResponse, status_code=201)
async def register(body: RegisterRequest, request: Request, db: AsyncSession = Depends(get_db)):
    r = await db.execute(select(User).where(User.email == body.email.lower()))
    if r.scalar_one_or_none(): raise HTTPException(status_code=400, detail="Email already registered")
    ...
    user = User(org_id=org.id, email=body.email.lower(), full_name=body.full_name,
        password_hash=hash_password(body.password), role=UserRole.ADMIN, is_verified=True)
    db.add(user); await db.flush()
    access = create_access_token({"sub": user.id, "org": org.id, "role": user.role.value})
    refresh = create_refresh_token(user.id)
    db.add(UserSession(user_id=user.id, token_hash=hash_token(refresh),
...
async def login(body: LoginRequest, request: Request, db: AsyncSession = Depends(get_db)):
    ...
    if not org or not org.is_active: raise HTTPException(status_code=403, detail="Org suspended")
...
async def refresh_token(body: RefreshRequest, db: AsyncSession = Depends(get_db)):
    try:
        ...
        user_id = payload["sub"]
    except: raise HTTPException(status_code=401, detail="Invalid refresh token")
    r = await db.execute(select(UserSession).where(
        UserSession.token_hash == hash_token(body.refresh_token), UserSession.user_id == user_id))
    session = r.scalar_one_or_none()
    if not session: raise HTTPException(status_code=401, detail="Session not found")
    ...
    db.add(UserSession(user_id=user.id, token_hash=hash_token(new_refresh),
        expires_at=datetime.now(timezone.utc).replace(tzinfo=None)))
...
async def me(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    return UserResponse(id=current_user.id, email=current_user.email,
        ...
        org_id=current_user.org_id, org_name=org.name if org else "",
        org_plan=org.plan.value if org else "free", is_verified=current_user.is_verified)

@router.post("/logout")
async def logout(body: RefreshRequest, current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    r = await db.execute(select(UserSession).where(
        UserSession.token_hash == hash_token(body.refresh_token), UserSession.user_id == current_user.id))
    session = r.scalar_one_or_none()
