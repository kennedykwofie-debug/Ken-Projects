...
from src.db.models import Organisation,User,UserRole
...
class OrgResponse(BaseModel):
    id:str;name:str;slug:str;plan:str;industry:Optional[str];country:Optional[str];is_active:bool;user_count:int=0
class UpdateOrgRequest(BaseModel):
    name:Optional[str]=None;industry:Optional[str]=None;country:Optional[str]=None;settings:Optional[dict]=None
class CreateUserRequest(BaseModel):
    email:str;password:str;full_name:Optional[str]=None;role:UserRole=UserRole.USER
...
async def get_my_org(current_user:User=Depends(get_current_user),db:AsyncSession=Depends(get_db)):
    ...
    return OrgResponse(id=org.id,name=org.name,slug=org.slug,plan=org.plan.value,industry=org.industry,country=org.country,is_active=org.is_active,user_count=uc.scalar() or 0)
...
async def update_my_org(body:UpdateOrgRequest,current_user:User=Depends(require_admin),db:AsyncSession=Depends(get_db)):
    ...
    return OrgResponse(id=org.id,name=org.name,slug=org.slug,plan=org.plan.value,industry=org.industry,country=org.country,is_active=org.is_active)
...
async def list_all_orgs(current_user:User=Depends(require_superadmin),db:AsyncSession=Depends(get_db)):
    ...
    return [OrgResponse(id=o.id,name=o.name,slug=o.slug,plan=o.plan.value,industry=o.industry,country=o.country,is_active=o.is_active) for o in r.scalars().all()]
...
async def suspend_org(org_id:str,current_user:User=Depends(require_superadmin),db:AsyncSession=Depends(get_db)):
    ...
    org=r.scalar_one_or_none()
    if not org:raise HTTPException(404,"Not found")
    org.is_active=False;return{"message":f"Org {org.name} suspended"}
...
async def list_users(current_user:User=Depends(require_admin),db:AsyncSession=Depends(get_db)):
    ...
    return [UserListResponse(id=u.id,email=u.email,full_name=u.full_name,role=u.role.value,is_active=u.is_active,last_login=u.last_login.isoformat() if u.last_login else None) for u in r.scalars().all()]
...
async def create_user(body:CreateUserRequest,current_user:User=Depends(require_admin),db:AsyncSession=Depends(get_db)):
    ...
    if r.scalar_one_or_none():raise HTTPException(400,"Email already exists")
    if body.role==UserRole.SUPERADMIN:raise HTTPException(403,"Cannot assign superadmin")
    user=User(org_id=current_user.org_id,email=body.email.lower(),full_name=body.full_name,password_hash=hash_password(body.password),role=body.role,is_verified=True)
    db.add(user);await db.flush()
    return UserListResponse(id=user.id,email=user.email,full_name=user.full_name,role=user.role.value,is_active=user.is_active,last_login=None)
...
async def update_user(user_id:str,body:UpdateUserRequest,current_user:User=Depends(require_admin),db:AsyncSession=Depends(get_db)):
    ...
    if user.id==current_user.id:raise HTTPException(400,"Cannot modify own account")
    if body.role==UserRole.SUPERADMIN:raise HTTPException(403,"Cannot assign superadmin")
    if body.full_name is not None:user.full_name=body.full_name
    if body.role is not None:user.role=body.role
    if body.is_active is not None:user.is_active=body.is_active
    return UserListResponse(id=user.id,email=user.email,full_name=user.full_name,role=user.role.value,is_active=user.is_active,last_login=user.last_login.isoformat() if user.last_login else None)
...
async def delete_user(user_id:str,current_user:User=Depends(require_admin),db:AsyncSession=Depends(get_db)):
    ...
    await db.delete(user);return{"message":"User removed"}
