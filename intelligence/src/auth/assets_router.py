"""Asset registration and management endpoints."""
...
class AssetResponse(BaseModel):
    id: str; type: str; value: str; label: Optional[str]
    tags: List[str]; is_active: bool; last_scanned: Optional[str]; scan_data: dict
...
async def list_assets(current_user: User = Depends(get_current_user), db: AsyncSession = Depends(get_db)):
    ...
    return [AssetResponse(id=a.id, type=a.type.value, value=a.value, label=a.label,
        tags=a.tags or [], is_active=a.is_active,
        last_scanned=a.last_scanned.isoformat() if a.last_scanned else None,
        scan_data=a.scan_data or {}) for a in r.scalars().all()]

@router.post("/", response_model=AssetResponse, status_code=201)
async def add_asset(body: AssetCreate, current_user: User = Depends(require_analyst), db: AsyncSession = Depends(get_db)):
    r = await db.execute(select(Asset).where(
        Asset.org_id == current_user.org_id, Asset.type == body.type,
        Asset.value == body.value.strip().lower()))
    if r.scalar_one_or_none(): raise HTTPException(status_code=400, detail="Asset already exists")
    asset = Asset(org_id=current_user.org_id, type=body.type,
        value=body.value.strip().lower(), label=body.label,
        tags=body.tags or [], created_by=current_user.id)
    db.add(asset); await db.flush()
    return AssetResponse(id=asset.id, type=asset.type.value, value=asset.value,
        label=asset.label, tags=asset.tags or [], is_active=asset.is_active,
        last_scanned=None, scan_data={})
