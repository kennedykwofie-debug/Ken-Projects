// LLM-Powered Trade Ideas â generates actionable ideas from sweep data + delta context
...
- Blend geopolitical, economic, and market signals â cross-correlate across domains
...
function compactSweepForLLM(data, delta, previousIdeas) {
  ...
  if (urgentPosts.length) {
    ...
    for (const p of urgentPosts) {
      ...
      const trimmed = text.length > remaining ? text.substring(0, remaining) + 'â¦' : text;
      lines.push(`- ${trimmed}`);
  ...
  if (delta?.summary) {
    ...
    if (delta.signals?.escalated?.length) {
      sections.push(`ESCALATED: ${delta.signals.escalated.map(s => `${s.label}: ${s.previous}â${s.current} (${(s.changePct||0) > 0 ? '+' : ''}${(s.changePct||0).toFixed(1)}%)`).join(', ')}`);
    }
