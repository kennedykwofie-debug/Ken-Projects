// Memory Manager â hot/cold storage for sweep history and alert tracking
// v2: Atomic writes, decay-based alert cooldowns, configurable retention

import { readFileSync, writeFileSync, mkdirSync, existsSync, renameSync, unlinkSync } from 'fs';
import { join } from 'path';
import { computeDelta } from './lib_delta_engine.mjs.mjs';

const MAX_HOT_RUNS = 3;

// Alert cooldown tiers â repeated signals get progressively longer suppression
...
export class MemoryManager {
  ...
  _loadHot() {
    ...
    console.warn('[Memory] No valid hot memory found â starting fresh');
    ...
    try {
      ...
      try {
        if (existsSync(this.hotPath)) {
          // Copy current â .bak (overwrite previous backup)
          renameSync(this.hotPath, bakPath);
        }
      } catch { /* backup failure is non-fatal */ }

      // 3. Atomic rename: .tmp â hot.json
  ...
  // âââ Alert Signal Tracking (Decay-Based) âââââââââââââââââââââââââââââââ
  ...
  // Compact data for storage â strip heavy arrays
