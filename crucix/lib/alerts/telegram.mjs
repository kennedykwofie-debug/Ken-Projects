// Telegram Alerter v2 â Multi-tier alerts, semantic dedup, two-way bot commands
...
// âââ Alert Tiers ââââââââââââââââââââââââââââââââââââââââââââââââââââââââââââ
// FLASH:    Immediate action required â market-moving, time-critical (e.g. war escalation, flash crash)
// PRIORITY: Important signal cluster â act within hours (e.g. rate surprise, major OSINT shift)
// ROUTINE:  Noteworthy change â FYI, no urgency (e.g. trend continuation, moderate delta)

const TIER_CONFIG = {
  FLASH:    { emoji: 'ð´', label: 'FLASH',    cooldownMs: 5 * 60 * 1000,  maxPerHour: 6 },
  PRIORITY: { emoji: 'ð¡', label: 'PRIORITY', cooldownMs: 30 * 60 * 1000, maxPerHour: 4 },
  ROUTINE:  { emoji: 'ðµ', label: 'ROUTINE',  cooldownMs: 60 * 60 * 1000, maxPerHour: 2 },
};
...
export class TelegramAlerter {
  constructor({ botToken, chatId }) {
    ...
    this._contentHashes = {};    // Semantic dedup: hash â timestamp
  ...
  // âââ Core Messaging âââââââââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  async evaluateAndAlert(llmProvider, delta, memory) {
    ...
    // 1. Gather new signals â filter already-alerted AND semantically duplicate
    const allSignals = [
      ...(delta.signals?.new || []),
    ...
    if (!evaluation?.shouldAlert) {
      console.log('[Telegram] No alert â', evaluation?.reason || 'no qualifying signals');
      return false;
    }
  ...
  // âââ Rule-Based Alert Fallback ââââââââââââââââââââââââââââââââââââââââ
  ...
  _ruleBasedEvaluation(signals, delta) {
    ...
    // FLASH: nuclear anomaly, or â¥3 critical signals across domains
    if (nukeSignal) {
      return {
    ...
    // FLASH: â¥2 critical signals AND they span multiple domains
    ...
    // PRIORITY: â¥2 high/critical signals in same direction
    ...
    // PRIORITY: â¥5 new OSINT posts (surge in conflict reporting)
    ...
    // ROUTINE: any critical signal OR â¥3 high signals
  ...
  // âââ Two-Way Bot Commands âââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  async _pollUpdates() {
    try {
      ...
      // Silent â polling failures are non-fatal
  ...
  async _handleMessage(msg) {
    ...
    if (command === '/mute') {
      ...
      await this.sendMessage(
        `ð Alerts muted for ${hours}h â until ${new Date(this._muteUntil).toLocaleTimeString()} UTC\nUse /unmute to resume.`,
        { chatId: replyChatId, replyToMessageId: msg.message_id }
      );
      return;
    }
    ...
    if (command === '/unmute') {
      ...
      await this.sendMessage(
        `ð Alerts resumed. You'll receive the next signal evaluation.`,
        { chatId: replyChatId, replyToMessageId: msg.message_id }
      );
      return;
    }
    ...
    if (command === '/alerts') {
      ...
      const lines = recent.map(a =>
        `${TIER_CONFIG[a.tier]?.emoji || 'âª'} ${a.tier} â ${new Date(a.timestamp).toLocaleTimeString()}`
      );
      await this.sendMessage(
        `ð *Recent Alerts (last ${recent.length})*\n\n${lines.join('\n')}`,
        { chatId: replyChatId, replyToMessageId: msg.message_id }
      );
      return;
    }
  ...
  // âââ Semantic Dedup âââââââââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  _signalKey(signal) {
    // Improved key generation â use content hash for text signals, structured key for metrics
  ...
  // âââ Prompt Engineering âââââââââââââââââââââââââââââââââââââââââââââââââ
...
### NO ALERT â suppress if:
- Routine scheduled data (NFP, CPI, FOMC minutes on expected dates) UNLESS the deviation from consensus is extreme (>2Ï)
...
### ð´ FLASH â immediate, life-of-portfolio risk:
...
FLASH requires: â¥2 corroborating sources across different domains (e.g. OSINT + market data + satellite)

### ð¡ PRIORITY â act within hours:
...
- Unexpected economic data (>1.5Ï miss on major indicator)
- New conflict front or ceasefire collapse confirmed by ACLED + Telegram
PRIORITY requires: â¥2 signals moving in same direction, at least 1 from hard data

### ðµ ROUTINE â informational, no urgency:
...
Respond with ONLY valid JSON:
{
  ...
  _buildSignalContext(signals, delta) {
    ...
    if (marketSignals.length > 0) {
      sections.push('ð MARKET SIGNALS:\n' + marketSignals.map(s =>
        `  ${s.label}: ${s.from} â ${s.to} (${s.pctChange > 0 ? '+' : ''}${s.pctChange?.toFixed(1) || s.change}${s.pctChange !== undefined ? '%' : ''})`
      ).join('\n'));
    }

    if (osintSignals.length > 0) {
      sections.push('ð¡ OSINT SIGNALS:\n' + osintSignals.map(s => {
        const post = s.item || s;
    ...
    if (conflictSignals.length > 0) {
      sections.push('âï¸ CONFLICT INDICATORS:\n' + conflictSignals.map(s =>
        `  ${s.label}: ${s.from} â ${s.to} (${s.direction})`
      ).join('\n'));
    }

    if (otherSignals.length > 0) {
      sections.push('ð OTHER:\n' + otherSignals.map(s =>
        `  ${s.label || s.key || s.reason}: ${s.from !== undefined ? `${s.from} â ${s.to}` : 'new signal'}`
      ).join('\n'));
    }

    sections.push(`\nð SWEEP DELTA: direction=${delta.summary.direction}, total=${delta.summary.totalChanges}, critical=${delta.summary.criticalChanges}`);

    return sections.join('\n\n');
  }

  // âââ Message Formatting âââââââââââââââââââââââââââââââââââââââââââââââââ
  ...
  _formatTieredAlert(evaluation, delta, tier) {
    ...
    if (evaluation.actionable && evaluation.actionable !== 'Monitor') {
      lines.push(``, `ð¡ *Action:* ${evaluation.actionable}`);
    }

    if (evaluation.signals?.length) {
      lines.push('', `Signals: ${evaluation.signals.join(' Â· ')}`);
    }
